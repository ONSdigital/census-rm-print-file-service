...
from test.integration_tests.utilities import build_test_messages, send_action_messages, ICHHQ_message_template, \
    ICL_message_template, P_OR_message_template, PPD1_3_message_template
...
def test_P_OR_I1(sftp_client):
    # Given
    ichhqe_messages, _ = build_test_messages(ICHHQ_message_template, 1, 'ICHHQE', 'P_OR_I1')
    send_action_messages(ichhqe_messages)
...
def test_P_OR_I2(sftp_client):
    # Given
    ichhqe_messages, _ = build_test_messages(ICHHQ_message_template, 1, 'ICHHQE', 'P_OR_I2')
    send_action_messages(ichhqe_messages)
...
def test_P_OR_I2W(sftp_client):
    # Given
    ichhqe_messages, _ = build_test_messages(ICHHQ_message_template, 1, 'ICHHQE', 'P_OR_I2W')
    send_action_messages(ichhqe_messages)
...
def test_P_OR_I4(sftp_client):
    # Given
    ichhqe_messages, _ = build_test_messages(ICHHQ_message_template, 1, 'ICHHQE', 'P_OR_I4')
    send_action_messages(ichhqe_messages)
...
def get_print_and_manifest_filenames(sftp, remote_directory, pack_code, max_attempts=10):
    for _attempt in range(max_attempts):
        fl = sftp.listdir(remote_directory)
        print(*fl, sep=", ")
        print('File list loaded, len: ', len(fl))
